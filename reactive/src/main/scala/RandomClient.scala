...
object RandomClient {
  def main( args:Array[String] ):Unit  = {
    ...
    val streamNumberB = RandomNumberObservable.blocking()
    val streamString = RandomStringObservable.nonBlocking()//.subscribe(a => println(s" --- ${a.getStr}"))
    def isEven(n:Int):Boolean = n % 2 == 0
    def isOdd(n:Int):Boolean = n % 2 != 0

    //streamNumberA.doOnCompleted(sys.exit(0)).subscribe( e=> println(e.getCount))
    //println("-----------")
    ...
    }).doOnCompleted(sys.exit(0)).subscribe( e => println(s" sum left values: ${e._1} - right values: ${e._2} "))
